...
export const fetchResources = async (token: string | null, size: string, page: string, search: string, orgUnits: string[], applicationCategory: string) => {

    const applicationCategoryParameter = applicationCategory.length > 0 ? `applicationcategory=${applicationCategory}` : undefined
    const response = await fetch(`${RESOURCE_API_URL}${BASE_PATH}/api/resources/v1?${applicationCategoryParameter}&size=${size}&page=${page}&search=${search}${orgUnits.length > 0 ? '&orgUnits=' + orgUnits : ""}`, {
