...
export async function action({ request }: ActionFunctionArgs) {
    ...
    return redirect(
        `${getResourceNewUserAssignmentUrl(Number(data.get('resourceRef')))}${prepareQueryParamsWithResponseCode(searchParams).length > 0 ? prepareQueryParamsWithResponseCode(searchParams) + '&responseCode=' + response.status : '?responseCode=' + response.status}`
    );
}
    ...
    return (
        <>
            <Modal
                ...
                onClose={() =>
                    navigate(
                        `${getResourceNewUserAssignmentUrl(Number(params.id))}${prepareQueryParamsWithResponseCode(searchParams)}`
                    )
                }
                header={{
                ...
                <Modal.Body>
                    <VStack gap="4">
                        ...
                        <BodyShort>Trykk lagre for å bekrefte tildelingen</BodyShort>
                    </VStack>
                </Modal.Body>
                <Modal.Footer>
                    <Form method={'POST'}>
                    ...
                    <Button
                        ...
                        onClick={() =>
                            navigate(
                                `${getResourceNewUserAssignmentUrl(Number(params.id))}${prepareQueryParams(searchParams)}`
                            )
                        }>
                        Avbryt
                    </Button>
