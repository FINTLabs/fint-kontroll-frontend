...
export async function loader({ params, request }: LoaderFunctionArgs) {
    ...
    /*const isTopUnit: boolean = resource.validForOrgUnits?.some((ou) => ou.isTopOrgunit) ?? false;
...
export default function NewAssignment() {
    ...
    return (
        <Tabs.Panel value="brukere">
            <TableToolbar
                FilterComponents={
                    <UserTypeFilter userTypeOptions={validForRoles} kodeverk={userTypesKodeverk} />
                }
...
export const handle = {
    breadcrumb: ({ params }: BreadcrumbParams) => (
        <Link to={getResourceNewUserAssignmentUrl(Number(params.id))} className={'breadcrumb-link'}>
            Brukertildeling
        </Link>
    ),
};
