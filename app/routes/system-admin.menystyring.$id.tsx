...
const SystemAdminMenuSettingsForm = () => {
    const {menuItems, roleMenuItems, source} = useLoaderData<typeof loader>()
    ...
    return (
        <VStack className={"tab-content-container"} padding={"4"} marginBlock={"8"}>
            <Heading size={"medium"}>Menypunkter for [{roleId}]</Heading>
            ...
            {menuItems.map((menuItem, index) => {
                return (
                    <Switch
                        key={index}
                        value={menuItem.id}
                        checked={roleMenuItems.some(roleMenuItem => roleMenuItem.id === menuItem.id)}
                        onChange={(e) => setCheckedValue(e.target.value, e.target.checked)}
                        disabled={roleId === "sa" || (source !== "gui" && (menuItem.text === "Innstillinger" || menuItem.text === "Opprett ny ressurs"))}
                    >
                        {menuItem.text}
                    </Switch>
                )
