...
import { createDeviceAssignment, createRoleAssignment } from '~/data/fetch-assignments';
import { fetchResourceById } from '~/data/fetch-resources';
import React, { useState } from 'react';
import { getDeviceGroupNewAssignmentUrl, getRoleNewAssignmentUrl } from '~/data/paths';
...
export default function AssignResourceToDevice() {
    ...
    return (
        <Modal
            ...
            onClose={() =>
                navigate(
                    `${getDeviceGroupNewAssignmentUrl(Number(params.id))}${prepareQueryParamsWithResponseCode(searchParams)}`
                )
            }
            header={{
        ...
        >
            ...
            <Modal.Footer>
                <Form method={'POST'}>
                    ...
                    <input value={params.id} type="hidden" name="deviceGroupRef" />
                ...
                <Button
                    ...
                    onClick={() =>
                        navigate(
                            `${getDeviceGroupNewAssignmentUrl(Number(params.id))}${prepareQueryParams(searchParams)}`
                        )
                    }
                >
                    Avbryt
