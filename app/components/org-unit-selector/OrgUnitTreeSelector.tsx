    ...
    const performFilteringOnSelectedOrgUnits = (orgUnitClicked: IUnitItem): IUnitItem[] => {
        ...
        const collectDescendants = (orgUnit: IUnitItem) => {
            orgUnit.childrenRef.forEach((childRef) => {
                const childOrgUnit: IUnitItem | undefined = newOrgUnitList.find(
                    (unit) => unit.organisationUnitId === childRef
                );
                if (childOrgUnit) {
        ...
        orgUnitClicked.childrenRef.forEach((childRef) => {
            const childIndex = updatedOrgUnits.findIndex(
                (unit) => unit.organisationUnitId === childRef
            );
            if (childIndex !== -1) {
        ...
        const uncheckDescendants = (orgUnit: IUnitItem) => {
            orgUnit.childrenRef.forEach((childRef) => {
                const childOrgUnit: IUnitItem | undefined = updatedOrgUnits.find(
                    (unit) => unit.organisationUnitId === childRef
                );
                if (childOrgUnit) {
