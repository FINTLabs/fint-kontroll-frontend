...
export const ResourceInfoBox = ({
    resource,
    userTypeKodeverk,
    isAdmin,
    licenseEnforcementKodeverk,
}: ResourceInfoBoxProps) => {
    return (
        <Box id="resourceInfoBox" className="info-box" padding="8" borderRadius="xlarge">
            <VStack gap={'4'}>
                <HStack align={'center'} justify={'center'} gap={'8'}>
                    ...
                    {isAdmin && <StatusTag status={resource.status} />}
                </HStack>
                            ...
                            {resource.validForRoles && (
                                <li>
                                    ...
                                    <BodyShort textColor="subtle">
                                        {resource.validForRoles
                                            .map((role) =>
                                                translateUserTypeToLabel(role, userTypeKodeverk)
                                            )
                                            .join(', ')}
                                    </BodyShort>
                                </li>
