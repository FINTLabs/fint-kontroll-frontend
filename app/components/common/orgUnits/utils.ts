...
export const getOrgUnitByChildrenRef = (
...
const getAllChildren = (unit: IUnitItem, allOrgUnits: IUnitItem[]): IUnitItem[] => {
    const collectChildren = (currentUnit: IUnitItem, collected: IUnitItem[]): IUnitItem[] => {
        const children = allOrgUnits.filter((u) =>
            currentUnit.childrenRef.includes(u.organisationUnitId)
        );
...
export const getOrgUnitAndAllNestedChildren = (orgUnits: IUnitItem[], allOrgUnits: IUnitItem[]) => {
    return Array.from(
        new Set(orgUnits.flatMap((unit) => [unit, ...getAllChildren(unit, allOrgUnits)]))
    );
};
