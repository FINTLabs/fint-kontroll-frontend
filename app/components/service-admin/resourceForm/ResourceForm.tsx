...
export const ResourceForm: React.FC<ResourseFormProps> = ({
    ...
    const [selectedOwnerOrgUnit, setSelectedOwnerOrgUnit] = useState<IUnitItem | null>(
        newResource.resourceOwnerOrgUnitId
            ? allOrgUnits.find(
                  (orgUnit) => orgUnit.organisationUnitId === newResource.resourceOwnerOrgUnitId
              ) || null
            : null
    );
    ...
    const [selectedValidForOrgUnits, setSelectedValidForOrgUnits] = useState<IUnitItem[]>(
        allOrgUnits
            .filter((orgUnit) =>
                newResource.validForOrgUnits.some(
                    (unit) => unit.orgUnitId === orgUnit.organisationUnitId
                )
            )
            .map((orgUnit) => {
                const resourceUnit = newResource.validForOrgUnits.find(
                    (unit) => unit.orgUnitId === orgUnit.organisationUnitId
                );
                return {
                    ...orgUnit,
